...
function updateExtensionState(updates, message) {
  // function to send updated state info to extension to be stored
...
async function sendMessageExtension(message, data) {
  ...
  await chrome.runtime
    .sendMessage({ message: message, data: data })
    .then((response) => {
      console.info(
        "state information sent to extension and got %s in response",
        response
      );
      resp = response;
    })
    .catch((error) => {
      console.warn(
        "Popup could not send message to extension error was %s",
        error
      );
    });
