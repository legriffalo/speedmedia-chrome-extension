...
async function getState() {
  ...
  let extensionState = data.Extension
    ? JSON.parse(data.Extension)
    : {
        x: 20,
        y: 20,
        wordsperminute: 300,
        ...
        audiospeed: 1,
        lastread:
          "In order to use the speed reader feature copy text on any browser page using ctrl+c then play using the button or press space",
      };
...
async function messageTab() {
  ...
  chrome.tabs
    .sendMessage(tab.id, { data: "yo" })
    .then((response) => {
      console.info(
        "Popup received response %s from tab with title '%s' and url %s",
        response,
        response.title,
        response.url
      );
    ...
    .catch((error) => {
      console.warn(
        "Popup could not send message to tab %s: error was %s",
        tab.title,
        error
      );
    });
